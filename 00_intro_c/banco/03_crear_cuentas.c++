...
void mostrar (Cuenta *c) {
	cout << "Titular: " << c->titular << "\t Nº de cuenta: " << c->numCuenta << "\t Saldo: " << c->saldo << endl;
...
void inicializarP (Cuenta * cuentas[5]) {
		string nombres[5] = {"juan","perico","andres","fulano","vengano"};
...
void mostrarArrayCuentas (Cuenta * cuentas [5]) {
	for (int i = 0; i< 5 && cuentas[i] != NULL; i++) {
		mostrar(cuentas[i]);
	}
}
...
void anyadirCuenta(Cuenta * cuentas [5]) {

	string titular, numCuenta;
	float saldo;
	
	cout << "Creación de nueva cuenta." << endl;
	...
	cout << "Ingreso inicial: ";
	cin >> saldo;
	
	Cuenta * cuenta = new Cuenta (numCuenta, titular, saldo);
...
int main (int argc, char *argv[]) {
	...
	inicializarP(cuentas);
	mostrarArrayCuentas(cuentas);
	
	anyadirCuenta(cuentas);
	mostrarArrayCuentas(cuentas);
	
	
	Cuenta* cuenta = seleccionarCuenta2(cuentas);
	cuenta->saldo += 100;
	mostrarArrayCuentas(cuentas);
